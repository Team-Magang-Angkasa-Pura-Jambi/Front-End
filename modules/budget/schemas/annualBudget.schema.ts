...
const requiredPositiveInt = (label: string) =>
  requiredPositiveNumber(label).int({ message: `${label} harus berupa bilangan bulat.` });
...
const parentBudgetSchema = baseBudgetSchema.extend({
  ...
  efficiency_tag: z.coerce
    ...
    .refine((val) => val > 0, { message: "Efficiency Tag harus lebih dari 0." }), 
...
const childBudgetSchema = baseBudgetSchema.extend({
  ...
  allocations: z
    ...
    .min(1, { message: "Anggaran periode wajib memiliki minimal satu alokasi meter." }),
...
export const annualBudgetFormSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.budgetType === "child") {
      const totalWeight = data.allocations.reduce((sum, item) => sum + (item.weight || 0), 0);
      
      if (Math.abs(totalWeight - 1) > 0.001) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: `Total bobot alokasi saat ini ${(totalWeight * 100).toFixed(1)}%. Total harus tepat 100%.`,
          path: ["allocations"],
        });
      }
    }
