...
export const FormReadingElectric = ({
  ...
  useEffect(() => {
    ...
    if (firstSuccessfulData) {
      const lastDateStr = firstSuccessfulData?.data?.data.session.reading_date;
      const lastDate = new Date(lastDateStr ?? new Date());
        ...
        <ScrollArea className="max-h-[70vh] overflow-y-auto p-4">
          ...
          <div className="space-y-4">
            {fields.map((field, index) => {
              const queryResult = lastReadingQueries[index];
              const lastVal = queryResult?.data?.data?.value;
              const lastDateStr =
                queryResult?.data?.data?.session?.reading_date;
